    ...
    def __init__(self, **_kwargs: Any) -> None:
        ...
        if not self._call_sync and not self._chat_session_sync and not self._call_artifact_sync:
            log_event(
                ...
                status="warning",
                details={"message": "No Supabase sync configured. Data will only persist to disk."},
            )
    ...
    def upsert_call_artifacts_direct(
        ...
        *,
        ...
        with timed_step(
            ...
            if settings.SUPABASE_VERBOSE_SYNC_LOGGING:
                audio_stats: Dict[str, Any] = {}
                for side, ap in audio_payload.items():
                    if not isinstance(ap, dict):
                        continue
                    audio_stats[side] = {
                        "bytes": int(ap.get("byte_count") or 0),
                        "stored": bool(ap.get("payload_b64")),
                        "truncated": bool(ap.get("truncated", False)),
                    }
                log_event(
                    "storage",
                    "call_artifacts_synced",
                    task_id=task_id,
                    details={
                        "transcript_turns": len(transcript or []),
                        "analysis_keys": sorted([str(key) for key in (analysis or {}).keys()])[:24],
                        "audio": audio_stats,
                        "reason": reason,
                    },
                )
