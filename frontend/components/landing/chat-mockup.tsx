        ...
        <div ref={ref} className="h-[260px] overflow-y-auto px-5 py-5 space-y-3">
          {messages.map((msg, i) => (
            <motion.div key={`${cycle}-${i}`} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}>
              {msg.role === 'status' ? (
                <div className="flex justify-center"><span className="text-[11px] text-emerald-600 font-medium bg-emerald-50 rounded-full px-3 py-1">{msg.text}</span></div>
              ) : msg.role === 'user' ? (
                <div className="flex justify-end"><div className="max-w-[78%] rounded-[18px] rounded-tr-md bg-gray-900 px-4 py-2 text-[13px] leading-[1.45] text-white">{msg.text}</div></div>
              ) : (
                <div className="flex justify-start"><div className="max-w-[78%] rounded-[18px] rounded-tl-md bg-gray-100 px-4 py-2 text-[13px] leading-[1.45] text-gray-900">{msg.text}</div></div>
              )}
            </motion.div>
          ))}
          {typing ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-start">
              <div className="rounded-[18px] rounded-tl-md bg-gray-100 px-4 py-2 flex items-center gap-[3px]">
                {[0, 1, 2].map((i) => (
