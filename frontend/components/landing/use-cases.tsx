          ...
          <Reveal delay={0.1} className="lg:col-span-3">
            <div className="rounded-2xl border border-gray-100 bg-white overflow-hidden">
              {[
                ...
                <div key={c.title} className={`flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-5 px-5 sm:px-6 py-4 sm:py-5 ${i > 0 ? 'border-t border-gray-100' : ''}`}>
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gray-50 text-gray-400">
                    <c.icon size={18} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-[14px] font-semibold text-gray-950 mb-1">{c.title}</h3>
                    <span className="text-[12px] text-gray-400 block sm:inline">{c.desc}</span>
